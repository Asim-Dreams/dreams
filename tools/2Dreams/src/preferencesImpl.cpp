...
preferencesImpl::preferencesImpl( QWidget* parent,  const char* name, bool modal, WFlags fl )
    : PreferencesDialog( parent, name, modal, fl )
{
    ...
    ToolButton_AvailableTags->setEnabled((DreamsDB::getInstance())->hasLoadedData());
    ...
    LineEdit_LogFile->setEnabled(!(DreamsDB::getInstance())->hasLoadedData());
    LineEdit_ADF->setEnabled(!(DreamsDB::getInstance())->hasLoadedData());
    CheckBox_ValidateADF->setEnabled(!(DreamsDB::getInstance())->hasLoadedData());
    LineEdit_ValidationCmd->setEnabled(!(DreamsDB::getInstance())->hasLoadedData() && pref->getPerformValidation());
    ToolButton_BrowseLog->setEnabled(!(DreamsDB::getInstance())->hasLoadedData());
    ToolButton_BrowseADF->setEnabled(!(DreamsDB::getInstance())->hasLoadedData());
    ToolButton_BrowseCmd->setEnabled(!(DreamsDB::getInstance())->hasLoadedData() && pref->getPerformValidation());

    if ((DreamsDB::getInstance())->hasLoadedData())
    {
...
preferencesImpl::knownTagsBtnClicked()
{
    ...
    strlist = (DreamsDB::getInstance())->getKnownTags();
    ...
    {
        dlg->insertItem(QString(str),draldb->getTagLongDesc(QString(str)));
    }
    dlg->exec();
